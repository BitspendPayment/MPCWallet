...
/// Phase 3: per-participant key-preserving REFRESH of V′ onto new participant ids.
/// Each holder (author, cosigner) runs `refresh_share_to_id`; the participant sums
/// the two `*(participant_id)` halves into `P_i` and the cosigner sums the two
/// `*(cosigner_id)` halves into `C_i`. Asserts: (1) {P_i,C_i} reconstruct the SAME
/// V′ (key unchanged), (2) pooling {P_1,P_2} does NOT reconstruct V′ (no collusion),
/// (3) {P_1,C_1} produce a valid V′ Schnorr signature (genuine Shamir 2-of-2).
#[test]
fn test_refresh_share_to_id_multi_participant() {
    let (old_kps, old_pkp) = run_full_dkg(2, 2);
    let (a_kp, c_kp, v_prime_pkp) = run_reshare_2party(&old_kps, &old_pkp);

    let a_id = a_kp.identifier.clone();
    let c_id = c_kp.identifier.clone(); // cosigner id, reused in each new sharing
    let id_set = [a_id.clone(), c_id.clone()];
    let v_prime = v_prime_pkp.verifying_key.point;

    let mut rng = OsRng;
    let p1_id = Identifier::derive(b"participant-bob").unwrap();
    let p2_id = Identifier::derive(b"participant-carol").unwrap();

    // Both holders refresh onto `part_id`; assemble {P_i, C_i}.
    let onboard = |part_id: &Identifier, rng: &mut OsRng| -> (Scalar, Scalar) {
        let slope_a = random_scalar(rng);
        let slope_b = random_scalar(rng);
        let (a_at_p, a_at_c) =
            dkg::refresh_share_to_id(&a_kp, &id_set, part_id, &c_id, &slope_a).unwrap();
        let (b_at_p, b_at_c) =
            dkg::refresh_share_to_id(&c_kp, &id_set, part_id, &c_id, &slope_b).unwrap();
        (a_at_p + b_at_p, a_at_c + b_at_c) // (P_i, C_i)
    };
    let (p1, c1) = onboard(&p1_id, &mut rng);
    let (p2, c2) = onboard(&p2_id, &mut rng);

    // (1) {P_i, C_i} at {p_i_id, c_id} reconstruct v′ ⇒ recover V′ (key UNCHANGED).
    let recover = |p_i: &Scalar, c_i: &Scalar, p_id: &Identifier| {
        let set = [p_id.clone(), c_id.clone()];
        let lam_p = lagrange_coeff_at_zero(p_id, &set);
        let lam_c = lagrange_coeff_at_zero(&c_id, &set);
        point::base_mul(&(lam_p * p_i + lam_c * c_i))
    };
    assert!(
        point::points_equal(&recover(&p1, &c1, &p1_id), &v_prime),
        "participant 1: {{P,C}} must reconstruct V′ (key unchanged)"
    );
    assert!(
        point::points_equal(&recover(&p2, &c2, &p2_id), &v_prime),
        "participant 2: {{P,C}} must reconstruct V′ (key unchanged)"
    );

    // (2) No collusion: {P_1, P_2} at {p1_id, p2_id} do NOT reconstruct v′.
    let set12 = [p1_id.clone(), p2_id.clone()];
    let pooled = lagrange_coeff_at_zero(&p1_id, &set12) * p1
        + lagrange_coeff_at_zero(&p2_id, &set12) * p2;
    assert!(
        !point::points_equal(&point::base_mul(&pooled), &v_prime),
        "pooling P_1 + P_2 must NOT reconstruct V′ (independent per-participant slopes)"
    );

    // (3) {P_1, C_1} sign + verify under V′ (genuine Shamir 2-of-2, no special case).
    let kp_p1 = KeyPackage {
        identifier: p1_id.clone(),
        secret_share: p1,
        verifying_share: point::base_mul(&p1),
        verifying_key: v_prime_pkp.verifying_key.clone(),
        min_signers: 2,
    };
    let kp_c1 = KeyPackage {
        identifier: c_id.clone(),
        secret_share: c1,
        verifying_share: point::base_mul(&c1),
        verifying_key: v_prime_pkp.verifying_key.clone(),
        min_signers: 2,
    };
    let pkp1 = PublicKeyPackage {
        verifying_shares: [
            (p1_id.clone(), point::base_mul(&p1)),
            (c_id.clone(), point::base_mul(&c1)),
        ]
        .into_iter()
        .collect(),
        verifying_key: v_prime_pkp.verifying_key.clone(),
    ...
    let msg = b"multi-user contract evtxo: participant 1 cooperative spend";
    let mut nonces = BTreeMap::new();
    let mut commitments = BTreeMap::new();
    for kp in [&kp_p1, &kp_c1] {
        let nonce = new_nonce(&mut rng, &kp.secret_share);
        commitments.insert(kp.identifier.clone(), nonce.commitments.clone());
        nonces.insert(kp.identifier.clone(), nonce);
    }
    ...
    let signature = aggregate(&signing_package, &shares, &pkp1).unwrap();
    signature
        .verify(&pkp1.verifying_key, msg)
        .expect("V′ signature from refreshed {P_1, C_1} must verify");
}
